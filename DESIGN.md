...
Replaying the byte-identical request the runner builds, ten times against `jev-1.13.0` (2026-09-20), held the label at `violation` 10/10 while probability moved 0.87-0.92 (sd 0.015) and confidence 0.82-0.88 (sd 0.019). The label is stable; the numbers the thresholds compare against are not. That rule's gate is 0.90/0.80, which sits inside the band: 4 of the 10 replays satisfy it and are a `violation`, the other 6 fall short and are a `review`. Identical evidence, opposite verdict, decided by sampling noise. For a blocking rule that is a red build on a plain re-run. Thresholds must therefore sit clear of where a rule lands in practice; a rule that repeatedly lands near its gate is miscalibrated, not marginal. Measure by replaying the runner's own request body - a hand-assembled payload with a different prompt framing lands somewhere else entirely.
